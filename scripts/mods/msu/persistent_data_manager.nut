this.persistent_data_manager <- {
	...
	function importModConfigFiles()
	{
		...
		foreach (file in persistentDirectory)
		{
			...
			::MSU.Mod.printWarning(format("Checking file, potential modID: '%s' and settingstype '%s'.", modID, settingsType), "persistence");
			this.setMod(modID);
	...
	{
		::MSU.Mod.printWarning(format("Loading setting '%s' for mod '%s'.", _settingID, _modID), "persistence");
